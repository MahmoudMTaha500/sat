...
methods: {
    getCountry(){
        axios.get("/sat-laravel/public/dashboard/country").then( response => this.countries = response.data.country);
return this.countries;
         },
         getcities: function (){
            var country_id = this.selected; 
            //  alert( country_id);
             axios.get("/sat-laravel/public/dashboard/getcities" , {
                 params:{
         ...
         addCity: function(){
           ...
           if(city && country_id){
                axios.post('/sat-laravel/public/dashboard/addCity',{name_ar: city, country_id:country_id}, {headers: {'X-CSRFToken': '{{ csrf_token }}',}})
                .then((response)=>{
